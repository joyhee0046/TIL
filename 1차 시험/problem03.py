####################################################
# Q3.정사피는 이번 프로젝트에서 회원가입 유효성 검사팀에 들어가게 되었다.
# 유효성 검사팀은 회원가입 과정에서 유저가 입력한 정보를 검사하는 프로그램을 만드는 팀이다.
# 신규 아이디 생성 시 아이디와 비밀번호 모두 빈 값이 입력되는 것을 방지하고자 한다.
# 사용자의 입력 정보인 user_data가 python의 dictionary형태로 들어온다고 할 때, 사용자가 입력한 아이디와 비밀번호 중 하나라도 비어있는 문자열이면 False 그렇지 않으면 True를 반환하는 is_user_data_valid 함수를 완성하시오.
############## 주의 ##############
